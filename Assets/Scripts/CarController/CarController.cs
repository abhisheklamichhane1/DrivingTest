...
public class CarController : MonoBehaviour
{
    ...
    public void gearOptiom()
    {
        if (carSetting.automaticGear)
        {
            carSetting.automaticGear = false;
        }
        else
        {
            carSetting.automaticGear = true;
        }
    }

    public void OnGUI()
    {
        if (GUI.Button(new Rect (20, 40, 200, 60), "Auto / Mannual Gear System"))
        {
            gearOptiom();
        }
    }
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.transform.root.GetComponent<CarController>())
        {
            ...
            collision.transform.root.GetComponent<CarController>().slip2 = Mathf.Clamp(collision.relativeVelocity.magnitude, 0.0f, 10.0f);
    ...
    void OnCollisionStay(Collision collision)
    {
        ...
        if (collision.transform.root.GetComponent<CarController>())
            collision.transform.root.GetComponent<CarController>().slip2 = 5.0f;
    ...
    {
          // speed of car
        speed = myRigidbody.velocity.magnitude * 4.7f;
        ...
        motorRPM = 0.95f * motorRPM + 0.05f * Mathf.Abs(rpm * carSetting.gears[gearBox.gear]);
        if (motorRPM > 5500.0f) motorRPM = 5200.0f;
        int index = (int)(motorRPM / efficiencyTableStep);
        if (index >= efficiencyTable.Length) index = efficiencyTable.Length - 1;
        if (index < 0) index = 0;
        float newTorque = curTorque * carSetting.gears[gearBox.gear] * efficiencyTable[index];
